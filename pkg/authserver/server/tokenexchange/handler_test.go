...
const testAgentSPIFFEID = "spiffe://toolhive.dev/ns/agents/sa/devops-agent"
...
// contextWithSPIFFEID returns a context containing the test agent's SPIFFE ID.
func contextWithSPIFFEID(t *testing.T) context.Context {
	t.Helper()

	id, err := spiffeid.FromString(testAgentSPIFFEID)
	require.NoError(t, err)
	...
	tests := []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, req *fosite.AccessRequest) {
				...
				// Verify the act claim contains the agent's SPIFFE ID.
